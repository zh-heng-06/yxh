...
def main() -> int:
    ...
    check(status["version"] == "1.1.2" and status["database"] == "ok" and "printer" in status and status["lanUrl"] and status["disk"]["freeGB"] > 0, "V1.1运行状态与磁盘接口", str(status["printer"]))
    ...
    owner.call(f"/api/devices/{first_id}/update", "POST", {"notes": "不应写入"}, expected=409)
    owner.call(f"/api/devices/{first_id}/photos", "POST", {"image": screenshot_url, "description": "不应新增"}, expected=409)
    check(True, "已出库设备资料与照片留档锁定")
