...
async function renderAuditEvents() {
  if (user.role !== "owner") { $("#audit-list").innerHTML = ""; return; }
  const rows = await api.auditEvents({action:$("#audit-action").value});
  $("#audit-list").innerHTML = rows.length ? rows.map(item => `<div class="audit-row ${item.success ? "" : "failed"}"><span><b>${esc(auditNames[item.action] || item.action)}</b> · ${esc(item.summary)}<small>${esc(item.actor_name || "系统")} · ${new Date(item.created_at).toLocaleString("zh-CN")} · ${esc(item.client_ip || "本机任务")}</small></span><i>${item.success ? "成功" : "失败"}</i></div>`).join("") : `<p class="empty-inline">暂无操作日志</p>`;
}

function renderAlerts(result) {
  const center = $("#alert-center");
  center.hidden = !result.items.length;
  $("#alert-capital").textContent = user.role === "owner" && result.agedCapital ? `30天以上库存占用 ${money(result.agedCapital)}` : "";
  $("#alert-list").innerHTML = result.items.map(item => `<button class="alert-item ${esc(item.severity)}" data-alert-scope="${esc(item.scope)}"><span><b>${esc(item.title)} · ${item.count}</b><small>${esc(item.message)}</small></span><i>查看</i></button>`).join("");
}

async function refresh() {
  const search = $("#search").value.trim();
  const [dash, list, alerts] = await Promise.all([
    api.dashboard(),
...
function showHandoff(handoff) {
  if (!handoff) return;
  $("#handoff-qr").src = `${handoff.qrUrl}?v=${Date.now()}`;
  $("#handoff-url").textContent = handoff.url;
  $("#handoff-open").href = handoff.url;
...
async function openDetail(deviceId) {
  ...
  if (latestSale) {
    ...
    $("#handoff-void").hidden = user.role !== "owner" || !handoff || handoff.status !== "active";
  }
...
$("#settings-button").addEventListener("click", async () => {
  ...
  $("#audit-block").hidden = user.role !== "owner";
  $("#settings-dialog").showModal();
  try { await Promise.all([refreshSystemStatus(), renderUsers(), renderBackups(), renderAuditEvents()]); } catch (error) { toast(error.message); }
});
$("#audit-refresh").addEventListener("click", () => renderAuditEvents().catch(error => toast(error.message)));
$("#audit-action").addEventListener("change", () => renderAuditEvents().catch(error => toast(error.message)));
...
$("#scan-sale-form").addEventListener("submit", async event => {
  ...
  try {
    const data = Object.fromEntries(new FormData(event.currentTarget));
    const result = await api.sell(scanCandidate.id, data);
    $("#scan-dialog").close();
    toast(result.handoffError || `出库成功：${scanCandidate.stock_code}`);
    await refresh();
    if (result.handoff) showHandoff(result.handoff);
  } catch (error) {
    $("#scan-error").textContent = error.message;
  } finally {
    button.disabled = false;
...
document.addEventListener("click", event => {
  const alertItem=event.target.closest("[data-alert-scope]");
  if(alertItem){const scope=alertItem.dataset.alertScope;if(scope){listScope=scope;$("#search").value="";document.querySelectorAll("[data-scope]").forEach(item=>item.classList.toggle("active",item.dataset.scope===scope));refresh().then(()=>$("#list-title").scrollIntoView({behavior:"smooth",block:"start"})).catch(error=>toast(error.message));}else{$("#settings-button").click();}return;}
...
$("#after-sales-create").addEventListener("click",async()=>{if(!currentDetail)return;const issue=prompt("客户反馈的问题：");if(!issue)return;try{await api.createAfterSales(currentDetail.id,{issue});toast("售后已登记");await refresh();await openDetail(currentDetail.id);}catch(error){toast(error.message);}});
$("#after-sales-list").addEventListener("click",async event=>{const button=event.target.closest("[data-resolve-after-sales]");if(!button)return;const resolution=prompt("处理结果（例如：更换电池、软件处理、退回检测）：");if(!resolution)return;const serviceCost=prompt("本次售后成本：","0");if(serviceCost===null)return;button.disabled=true;try{await api.resolveAfterSales(button.dataset.resolveAfterSales,{resolution,serviceCost});toast("售后处理已完成");await refresh();await openDetail(currentDetail.id);}catch(error){button.disabled=false;toast(error.message);}});
$("#handoff-reissue").addEventListener("click",async()=>{if(!currentDetail?.latestSale)return;try{const result=await api.reissueHandoff(currentDetail.latestSale.id);toast(currentDetail.handoff?"新交接码已生成，旧链接已失效":"交接卡已生成");await openDetail(currentDetail.id);showHandoff(result.handoff);}catch(error){toast(error.message);}});
$("#handoff-void").addEventListener("click",async()=>{if(!currentDetail?.latestSale||!confirm("确认作废这张顾客交接卡？顾客的旧链接将无法再打开。"))return;try{await api.voidHandoff(currentDetail.latestSale.id);toast("交接卡已作废");await openDetail(currentDetail.id);}catch(error){toast(error.message);}});
$("#handoff-copy").addEventListener("click",async()=>{const value=$("#handoff-url").textContent;try{await navigator.clipboard.writeText(value);toast("交接卡链接已复制");}catch{prompt("复制交接卡链接：",value);}});
...
$("#sale-form").addEventListener("submit", async event => {
  ...
  const data = Object.fromEntries(new FormData(event.currentTarget));
  try {
    const result = await api.sell(data.deviceId, data);
    $("#sale-dialog").close();
    toast(result.handoffError || "出库成功");
    await refresh();
    if (result.handoff) showHandoff(result.handoff);
  } catch (error) {
